...
export function renderProjectWorkbench(state: ProjectWorkbenchState): string {
  ...
  return `
    <div class="feature-layout">
      ...
      <section class="panel" data-testid="projects-apply-result"><h2>${t("feature.projects.applyResult")}</h2>${state.errors.applyResult ? `<div class="error-state">${escapeHtml(state.errors.applyResult)}</div>` : ""}${state.applyResult ? renderRows([{ label: localize("Success", "是否成功"), value: String(state.applyResult.success) }, { label: localize("Message", "消息"), value: state.applyResult.message }]) : `<div class="empty">${t("state.notChecked")}</div>`}</section>

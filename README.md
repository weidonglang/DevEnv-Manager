...
[项目主页](https://github.com/weidonglang/DevEnv-Manager) · [Release 下载](https://github.com/weidonglang/DevEnv-Manager/releases) · [完整操作手册](docs/user-guide.md) · [Issue #35–#42 实现说明](docs/issues-35-42.md) · [问题反馈](https://github.com/weidonglang/DevEnv-Manager/issues)

面向 Windows 的开发环境诊断器与安全操作面板。当前版本：**1.4.0**。
...
1.4 起，用户级 `JAVA_HOME` 会写入类似 `D:\DevEnvManager\current\jdk` 的真实绝对路径，不再写 `%DEVENV_HOME%\current\jdk`。这是为了兼容不会二次展开环境变量的 Nacos、Maven、Gradle 和部分脚本；`DEVENV_HOME` 与 PATH 的受管条目仍会保留。
...
- `tauri\src-tauri\target\release\bundle\nsis\DevEnv Manager_1.4.0_x64-setup.exe`
- `tauri\src-tauri\target\release\bundle\msi\DevEnv Manager_1.4.0_x64_en-US.msi`
...
| 文件 | 1.0.0 | Phase 1 | 1.1.0 | 1.3.0 | 1.4.0 | 1.4.0 相对 1.3.0 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `dailytools-tauri.exe` | 4,758,016 B | 4,726,272 B | 4,826,624 B | 5,047,808 B | 5,296,128 B | +248,320 B |
| `devenv.exe` | 2,073,600 B | 2,066,944 B | 2,102,784 B | 2,088,960 B | 2,148,352 B | +59,392 B |

Phase 2 没有单独发布可复现的 release 二进制，因此使用最近发布的 1.1.0 作为更严格基线。1.4.0 继续不引入大型依赖；Phase 4 相对 1.3.0 主程序仅增加 248,320 B，明显低于 15 MB 体积上限。
...
Maven 和 Gradle 需要有效 JDK。1.4 起用户级 `JAVA_HOME` 写入真实绝对路径；受管命令执行时也会注入已验证的绝对 JDK 路径，安装或切换后会重新验证。Maven/Gradle 已存在目录时，安装按钮会重新登记、切换 current 并验证，而不是直接报“已安装”。
